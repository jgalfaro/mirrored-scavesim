...
public class Simulator implements Runnable{
	...
	public static Vector<Node> nodeList3D;
	...
	public static Node actualGlobalNode;
	...
	public Simulator(Record pRecord, int pNumNodes, int pNumAgents,	double x, double y, double z, 
			...
			float agentSize, float nodeSize, int numbLandmarkToCross, String shortest_path
			) throws IOException{
		...
		this.nodeList3D = new Vector<Node>();
	    ...
	    if(numbLandmarkToCross <= landmarkListsize && numbLandmarkToCross >= 2) {
			...
			for (int i = 0; i < this.initialNumAgents; i++){
				...
				Agent localAgent = new Agent(i,0.9, this.dataCollector, 5); // MAXSPEED //5ms, 10ms, 15ms e 20 ms
	...
	public void run(){
		...
		while(i == 0){
			...
			actualGlobalNode = this.agentList.get(0).getCurrentNode();			
		...
		String path = Paths.get("").toAbsolutePath().toString() + "/results/" + this.output;
		String path_cspt = Paths.get("").toAbsolutePath().toString() + "/results/" + this.output.split("\\.")[0] + "-consumption.txt";
