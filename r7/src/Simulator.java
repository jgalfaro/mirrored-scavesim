...
public class Simulator implements Runnable {
	...
	private void initAgents(int initialNumAgents, int initialNumNodes) {
		...
		for (int index = 0; index < initialNumAgents; index++) {
			...
			do {
				unique = true;
				//Check if exists value initialized
                                if(Main.initAgents!=null){
                                    if(index<Main.initAgents.size()){
					initialPosition = Main.initAgents.get(index);
                                    }else{
					//Else random vale
			...
			initialNodes.add(initialPosition);
	...
	private void initBHPosition(int numBH, int numNodes) {
		...
		for (int i = 0; i < numBH; i++) {
			...
			do {
				unique = true;
                                if(Main.initBH!=null){
                                    if(i<Main.initBH.size()){
					tmpBH = Main.initBH.get(i);
                                    }else{
		...
		while (true) {
			...
			if (hasAgentsExecuting()) {// we stop the simulation
				...
				while (continueP || status == Status.FINISHED) {
					...
					Thread.yield();
                                        //Joaquin
                                        //force_exit_for_automatic_tests
                                        if(loopCounter>1000){
                                            System.out.println("forced exit after "+loopCounter+" loops");
                                            System.exit(0);
                                        }else{
