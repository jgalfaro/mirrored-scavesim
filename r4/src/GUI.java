...
public class GUI{
	static boolean records=false;//TODO: set true/false from GUI, to log/unlog records
	static float f = 0.0F;
	static int m = -1;
	static Simulator lSim;
	static JFrame mainFrame = new JFrame("Sample GUI for the simulator");
	...
	public static void create(final int nStations, final int nAgents, final int xNet, final int yNet){
		...
		lSim =  new Simulator(new Record(records), nStations, nAgents, xNet, yNet,0, localClass, f, m);
		...
		drawButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//				JRootPane jroot=(JRootPane)mainFrame.getComponent(0);
				mainFrame.getContentPane().remove(mainFrame.getContentPane().getComponent(1));
				//				JLayeredPane jlayered=(JLayeredPane)jroot.remove(1);
				//				JPane jlayered=(JLayeredPane)jroot.getComponent(1);
				//				jlayered.remove(1);
